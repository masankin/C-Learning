...
namespace cocostudio
{
    ...
    Node* LayoutReader::createNodeWithFlatBuffers(const flatbuffers::Table *layoutOptions)
    {
		...
		logStr.append(layout->getName() + "\"");
		logStr.append(" ActionTag=\"");
		logStr.append(ConvertToString(layout->getActionTag()) + "\"");
		if (layout->getCallbackType().compare("") != 0)
		{
			logStr.append(" CallBackType=\"");
			logStr.append(layout->getCallbackType() + "\"");
			logStr.append(" CallBackName=\"");
			logStr.append(layout->getCallbackName() + "\"");
		}
		
		logStr.append(" Tag=\"");
		logStr.append(ConvertToString(layout->getTag()) + "\"");
		logStr.append(" Alpha=\"");
		logStr.append(ConvertToString((int)layout->getOpacity()) + "\"");
		logStr.append(" VisibleForFrame=\"");
		logStr.append((layout->isVisible() ? "True\"" : "False\""));
		logStr.append(" IconVisible=\"False\"");
		logStr.append(" TouchEnable=\"");
		logStr.append((layout->isTouchEnabled() ? "True\"" : "False\""));
		logStr.append(" FlipX=\"");
		logStr.append((layout->isFlippedX() ? "True\"" : "False\""));
		logStr.append(" FlipY=\"");
		logStr.append((layout->isFlippedY() ? "True\"" : "False\""));
		logStr.append(" ctype=\"PanelObjectData\">\n");
		log("%s", logStr.c_str());
