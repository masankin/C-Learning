...
//        bool bt = DICTOOL->checkObjectExist_json(options, P_BarFileName);
    ...
    Node* SliderReader::createNodeWithFlatBuffers(const flatbuffers::Table *sliderOptions)
    {
		...
		logStr.append(slider->getName() + "\"");
		logStr.append(" ActionTag=\"");
		logStr.append(ConvertToString(slider->getActionTag()) + "\"");
		if (slider->getCallbackType().compare("") != 0)
		{
			logStr.append(" CallBackType=\"");
			logStr.append(slider->getCallbackType() + "\"");
			logStr.append(" CallBackName=\"");
			logStr.append(slider->getCallbackName() + "\"");
		}
		logStr.append(" Tag=\"");
		logStr.append(ConvertToString(slider->getTag()) + "\"");
		logStr.append(" Alpha=\"");
		logStr.append(ConvertToString((int)slider->getOpacity()) + "\"");
		logStr.append(" VisibleForFrame=\"");
		logStr.append((slider->isVisible() ? "True\"" : "False\""));
		logStr.append(" IconVisible=\"False\"");
		logStr.append(" TouchEnable=\"");
		logStr.append((slider->isTouchEnabled() ? "True\"" : "False\""));
		logStr.append(" FlipX=\"");
		logStr.append((slider->isFlippedX() ? "True\"" : "False\""));
		logStr.append(" FlipY=\"");
		logStr.append((slider->isFlippedY() ? "True\"" : "False\""));
		logStr.append(" ctype=\"SliderObjectData\">\n");
		//

		logStr.append("<BackGroundData ");
		logStr.append(" Type=\"Normal\"");
		logStr.append(" Path=\"");
		logStr.append(imageFileName + "\"");
		logStr.append(" Plist=\"\"/>\n");

		logStr.append("<ProgressBarData ");
		logStr.append(" Type=\"Normal\"");
		logStr.append(" Path=\"");
		logStr.append(normalFileName + "\"");
		logStr.append(" Plist=\"\"/>\n");
