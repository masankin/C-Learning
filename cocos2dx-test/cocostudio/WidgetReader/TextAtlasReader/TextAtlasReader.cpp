...
//        bool scm = DICTOOL->checkObjectExist_json(options, P_StartCharMap);
    ...
    Node* TextAtlasReader::createNodeWithFlatBuffers(const flatbuffers::Table *textAtlasOptions)
    {
		...
		logStr.append(textAtlas->getName() + "\"");
		logStr.append(" ActionTag=\"");
		logStr.append(ConvertToString(textAtlas->getActionTag()) + "\"");
		if (textAtlas->getCallbackType().compare("") != 0)
		{
			logStr.append(" CallBackType=\"");
			logStr.append(textAtlas->getCallbackType() + "\"");
			logStr.append(" CallBackName=\"");
			logStr.append(textAtlas->getCallbackName() + "\"");
		}
		logStr.append(" Tag=\"");
		logStr.append(ConvertToString(textAtlas->getTag()) + "\"");
		logStr.append(" Alpha=\"");
		logStr.append(ConvertToString((int)textAtlas->getOpacity()) + "\"");
		logStr.append(" VisibleForFrame=\"");
		logStr.append((textAtlas->isVisible() ? "True\"" : "False\""));
		logStr.append(" IconVisible=\"False\"");
		logStr.append(" TouchEnable=\"");
		logStr.append((textAtlas->isTouchEnabled() ? "True\"" : "False\""));
		logStr.append(" FlipX=\"");
		logStr.append((textAtlas->isFlippedX() ? "True\"" : "False\""));
		logStr.append(" FlipY=\"");
		logStr.append((textAtlas->isFlippedY() ? "True\"" : "False\""));
		logStr.append(" CharWidth=\"");
		logStr.append(ConvertToString(itemWidth) + "\"");
		logStr.append(" CharHeight=\"");
		logStr.append(ConvertToString(itemHeight) + "\"");
		logStr.append(" LabelText=\"");
		logStr.append(stringValue + "\"");
		logStr.append(" StartChar=\"");
		logStr.append(ConvertToString(options->startCharMap()->c_str()) + "\"");
		logStr.append(" ctype=\"TextAtlasObjectData\">\n");
		//

		logStr.append("<LabelAtlasFileImage_CNB ");
		logStr.append(" Type=\"Default\"");
		logStr.append(" Path=\"");
		logStr.append(ConvertToString(cmfPath) + "\"");
		logStr.append(" Plist=\"\"/>\n");
