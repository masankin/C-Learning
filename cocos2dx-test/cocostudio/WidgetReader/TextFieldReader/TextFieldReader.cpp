...
//        }
    ...
    Node* TextFieldReader::createNodeWithFlatBuffers(const flatbuffers::Table *textFieldOptions)
    {
		...
		logStr.append(textField->getName() + "\"");
		logStr.append(" ActionTag=\"");
		logStr.append(ConvertToString(textField->getActionTag()) + "\"");
		if (textField->getCallbackType().compare("") != 0)
		{
			logStr.append(" CallBackType=\"");
			logStr.append(textField->getCallbackType() + "\"");
			logStr.append(" CallBackName=\"");
			logStr.append(textField->getCallbackName() + "\"");
		}
		logStr.append(" Tag=\"");
		logStr.append(ConvertToString(textField->getTag()) + "\"");
		logStr.append(" Alpha=\"");
		logStr.append(ConvertToString((int)textField->getOpacity()) + "\"");
		logStr.append(" VisibleForFrame=\"");
		logStr.append((textField->isVisible() ? "True\"" : "False\""));
		logStr.append(" IconVisible=\"False\"");
		logStr.append(" TouchEnable=\"");
		logStr.append((textField->isTouchEnabled() ? "True\"" : "False\""));
		logStr.append(" FlipX=\"");
		logStr.append((textField->isFlippedX() ? "True\"" : "False\""));
		logStr.append(" FlipY=\"");
		logStr.append((textField->isFlippedY() ? "True\"" : "False\""));
		logStr.append(" LabelText=\"");
		logStr.append(textField->getString() + "\"");
		logStr.append(" PlaceHolderText=\"");
		logStr.append(textField->getPlaceHolder() + "\"");
		logStr.append(" MaxLengthText=\"");
		logStr.append(ConvertToString(textField->getMaxLength()) + "\"");
		logStr.append(" FontSize=\"");
		logStr.append(ConvertToString(textField->getFontSize()) + "\"");
		logStr.append(" IsCustomSize=\"");
		logStr.append((true ? "True\"" : "False\""));
		logStr.append(" ctype=\"TextFieldObjectData\">\n");
		//

		logStr.append("<FontResource ");
		logStr.append(" Type=\"Normal\"");
		logStr.append(" Path=\"");
		logStr.append(textField->getFontName() + "\"");
		logStr.append(" Plist=\"\"/>\n");
